CREATE OR REPLACE VIEW `iptv.error_fct_incr_vw` AS
...
INNER JOIN `IPTV`.`ERROR_FCT` `F`ON `f`.`event_date` = `r`.`raw_event_date`AND `f`.`header`.`UUID` = `r`.`uuid`WHERE `r`.`rnk` = 1
