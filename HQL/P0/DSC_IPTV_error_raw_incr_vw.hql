CREATE OR REPLACE VIEW `iptv.error_raw_incr_vw` AS
...
ON `flt`.`iptv_pipeline` = `ref`.`iptv_pipeline`AND `flt`.`job_execution_range` = `ref`.`job_execution_range`
...
ON `a`.`iptv_pipeline`         = `b`.`iptv_pipeline`AND `a`.`job_execution_range` = `b`.`job_execution_range`AND `a`.`exec_dt` = `b`.`exec_dt`
WHERE `b`.`total_success_cnt` >= `a`.`pipeline_cnt`AND `a`.`job_name` IN
...
FROM (SELECT`r`.`header`.`TIMESTAMP`, `r`.`header`.`UUID`, `r`.`header`.`HOSTNAME`, `r`.`header`.`MONEY`.`TRACEID`, `r`.`header`.`MONEY`.`SPANID`, `r`.`header`.`MONEY`.`PARENTSPANID`, `r`.`header`.`MONEY`.`SPANNAME`, `r`.`header`.`MONEY`.`APPNAME`, `r`.`header`.`MONEY`.`STARTTIME`, `r`.`header`.`MONEY`.`SPANDURATION`, `r`.`header`.`MONEY`.`SPANSUCCESS`, `r`.`header`.`MONEY`.`NOTES`,`r`.`device`.`RECEIVERID`, `r`.`device`.`DEVICEID`, `r`.`device`.`DEVICESOURCEID`, `r`.`device`.`ACCOUNT`, `r`.`device`.`ACCOUNTSOURCEID`, `r`.`device`.`BILLINGACCOUNTID`, `r`.`device`.`MACADDRESS`,`r`.`device`.`ECMMACADDRESS`, `r`.`device`.`FIRMWAREVERSION`, `r`.`device`.`DEVICETYPE`, `r`.`device`.`MAKE`, `r`.`device`.`MODEL`, `r`.`device`.`PARTNER`, `r`.`device`.`IPADDRESS`, `r`.`device`.`UTCOFFSET`,`r`.`device`.`POSTALCODE`,`r`.`eventtimestamp`, `r`.`errorcode`, `r`.`errordescription`, `r`.`isvisible`, `r`.`isstartup`, `r`.`hdp_file_name`, `r`.`hdp_create_ts`, `r`.`hdp_update_ts`, `r`.`received_date`,
  ROW_NUMBER() OVER (PARTITION BY `r`.`header`.`UUID` ORDER BY `r`.`header`.`TIMESTAMP` DESC) AS `RNK`,TO_DATE(FROM_UNIXTIME(CAST(`r`.`header`.`TIMESTAMP`/1000 AS BIGINT))) AS `RAW_EVENT_DATE`FROM CFW, `IPTV`.`ERROR_RAW` `R`
WHERE `r`.`hdp_create_ts` > `cfw`.`min_value_ts`
AND `r`.`hdp_create_ts`  <= `cfw`.`max_value_ts`
AND `r`.`received_date`  >= `cfw`.`min_value`AND `r`.`received_date` <= `cfw`.`max_value`) `COMPACT`
WHERE `compact`.`rnk`     = 1;
