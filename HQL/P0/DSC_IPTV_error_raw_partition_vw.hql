CREATE OR REPLACE VIEW `iptv.error_raw_partition_vw` AS
...
SELECT `r`.`header`.`TIMESTAMP`,
  ...
  `IPTV`.`ERROR_RAW` `R`WHERE TO_DATE(`r`.`received_date`) >= `cfw`.`min_value`AND TO_DATE(`r`.`received_date`) < `cfw`.`max_value`;
