...
SELECT `compact`.`timestamp`,
  ...
  (SELECT `r`.`header`.`TIMESTAMP`,
    ...
    `IPTV`.`ERROR_RAW` `R`WHERE TO_DATE(FROM_UNIXTIME(CAST(`r`.`header`.`TIMESTAMP`/1000 AS BIGINT))) >= `cfw`.`min_value`AND TO_DATE(FROM_UNIXTIME(CAST(`r`.`header`.`TIMESTAMP`/1000 AS BIGINT))) < `cfw`.`max_value`
