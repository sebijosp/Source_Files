CREATE OR REPLACE VIEW IPTV.IP_PLAYBACK_RAW_INCR_VW AS
WITH REF_TBL AS (
SELECT
JOB_NAME           ,
JOB_TYPE           ,
...
WHERE RNK = 1
--AND RAW_EVENT_DATE >  TO_DATE(DATE_SUB(CURRENT_DATE(),9))
AND RAW_EVENT_DATE >= TO_DATE(DATE_SUB(CURRENT_DATE(),30))
AND (PROVIDERID IS NULL OR UPPER(PROVIDERID) NOT LIKE 'PPV_%')
;
