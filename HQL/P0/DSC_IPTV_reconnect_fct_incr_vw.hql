CREATE OR REPLACE VIEW `iptv.reconnect_fct_incr_vw` AS
...
AND `ref_tbl`.`iptv_source_table`   = 'IPTV.RECONNECT_FCT'),
...
ON `a`.`iptv_pipeline` = `b`.`iptv_pipeline`AND `a`.`job_execution_range` = `b`.`job_execution_range`AND `a`.`exec_dt` = `b`.`exec_dt`WHERE `b`.`total_success_cnt` >= `a`.`pipeline_cnt`
AND `a`.`job_name`    IN
...
INNER JOIN `IPTV`.`RECONNECT_FCT` `F`ON `f`.`event_date` = `r`.`raw_event_date`AND `f`.`header`.`UUID` = `r`.`uuid`
WHERE `r`.`rnk`                                          = 1
