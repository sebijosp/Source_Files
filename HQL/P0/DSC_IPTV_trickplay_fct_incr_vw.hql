CREATE OR REPLACE VIEW `iptv.trickplay_fct_incr_vw`
             AS
...
INNER JOIN `IPTV`.`TRICKPLAY_FCT` `F`ON `f`.`event_date` = `r`.`raw_event_date`
AND `f`.`header`.`UUID`                                  = `r`.`uuid`WHERE `r`.`rnk` = 1
