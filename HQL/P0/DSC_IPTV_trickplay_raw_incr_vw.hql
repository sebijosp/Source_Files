CREATE OR REPLACE VIEW `iptv.trickplay_raw_incr_vw`
             AS
WITH REF_TBL AS
...
ON `a`.`iptv_pipeline`         = `b`.`iptv_pipeline`AND `a`.`job_execution_range` = `b`.`job_execution_range`
...
SELECT `compact`.`timestamp`,
  ...
  (SELECT `r`.`header`.`TIMESTAMP`,
    ...
    `IPTV`.`TRICKPLAY_RAW` `R`WHERE `r`.`hdp_create_ts` > `cfw`.`min_value_ts`AND `r`.`hdp_create_ts` <= `cfw`.`max_value_ts`
