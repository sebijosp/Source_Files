CREATE OR REPLACE VIEW `iptv.tune_raw_incr_vw`
             AS
WITH REF_TBL AS
  ...
  (SELECT `ref_tbl`.`job_name` ,
    ...
    `ref_tbl`.`pipeline_cnt`FROM REF_TBL
...
ON `ref`.`job_name`         = `ctl`.`job_name`AND `ctl`.`execution_status` = 'SUCCEEDED'
...
ON `rcf`.`job_name` = `flt`.`iptv_job_name`WHERE `rcf`.`execution_status` = 'SUCCEEDED'),
...
ON `etl`.`glgn_job_name`  = `ctl`.`iptv_job_name`WHERE `etl`.`etl_min_value` >= `ctl`.`maxval_of_last_fin_cycle`
...
SELECT `compact`.`timestamp`,
  ...
  (SELECT `r`.`header`.`TIMESTAMP`,
    ...
    `IPTV`.`TUNE_RAW` `R`WHERE `r`.`hdp_create_ts` > `cfw`.`min_value_ts`AND `r`.`hdp_create_ts` <= `cfw`.`max_value_ts`
