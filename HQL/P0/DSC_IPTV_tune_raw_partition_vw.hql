CREATE or REPLACE VIEW `iptv.tune_raw_partition_vw` AS
...
GROUP BY `a`.`job_name`,`a`.`iptv_job_name`),
  CFW AS
  (SELECT COALESCE(TO_DATE(`ctl`.`rundt_of_last_fin_cycle`),TO_DATE(DATE_SUB(CURRENT_DATE(),1))) AS `MIN_VALUE`,
    CURRENT_DATE()                                                                               AS `MAX_VALUE`FROM REF_FLTR REF
...
SELECT `r`.`header`.`TIMESTAMP`,
  ...
  `r`.`hdp_create_ts` ,
  `r`.`hdp_update_ts` ,
  `r`.`received_date`FROM CFW,
  `IPTV`.`TUNE_RAW` `R`
WHERE TO_DATE(`r`.`received_date`) >= `cfw`.`min_value`AND TO_DATE(`r`.`received_date`) < `cfw`.`max_value`;
