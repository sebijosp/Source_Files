...
WITH REF_TBL AS
  (SELECT `nonetl_iptv_job_relatn_ref`.`job_name` ,
    ...
    COUNT(`nonetl_iptv_job_relatn_ref`.`job_name`) OVER (PARTITION BY `nonetl_iptv_job_relatn_ref`.`iptv_pipeline`, `nonetl_iptv_job_relatn_ref`.`job_execution_range`) AS `PIPELINE_CNT`FROM `IPTV`.`NONETL_IPTV_JOB_RELATN_REF`
...
AND `ref_tbl`.`iptv_source_table`   = 'IPTV.TUNE_RAW'),
...
INNER JOIN `IPTV`.`NONETL_JOB_EXECUTION_CNTRL` `CTL`ON `ref`.`job_name` = `ctl`.`job_name`AND `ctl`.`execution_status` = 'SUCCEEDED'
...
FROM
  (SELECT `r`.`header`.`TIMESTAMP`,
    ...
    `IPTV`.`TUNE_RAW` `R`WHERE TO_DATE(FROM_UNIXTIME(CAST(`r`.`header`.`TIMESTAMP`/1000 AS BIGINT))) >= `cfw`.`min_value`AND TO_DATE(FROM_UNIXTIME(CAST(`r`.`header`.`TIMESTAMP`/1000 AS BIGINT))) < `cfw`.`max_value`
