CREATE OR REPLACE VIEW `iptv`.`app_events_fct_incr_vw` AS WITH REF_TBL AS (
SELECT
...
AND   `ref_tbl`.`iptv_source_table`   = 'IPTV.APP_EVENTS_FCT'
),
BDM_CONTROL AS (
SELECT
...
FROM RAW_FILTERED R
INNER JOIN `IPTV`.`APP_EVENTS_FCT` `F`
