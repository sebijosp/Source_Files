...
--0.2 - danchang - 3/30/2016 - updated syntax for ext_cdrdm.v_MT_LD_UCC_temp4 & 5, cdrdm.v_MT_LD_UCC
--0.3 - danchang - 4/06/2016 - updated view ext_cdrdm.v_MT_LD_UCC_NPA_NXX_LR by changing table ext_ela_v21.NPA_NXX_LR to ext_ela_v21.NPA_NXX_LR_GG
--0.4 - danchang - 4/29/2016 - updated VerticalCoordinate -> Vertical_Coordinate (Horizontal as well) and ext_ela_v21 -> ela_v21
...
--modified join prediction route_id derivtion logic in ext_cdrdm.v_MT_LD_UCC_temp2 data load as part of UCC Phase 2 Requirement
...
DROP VIEW IF EXISTS v_MT_LD_UCC_NPA_NXX_STATE_MPS;
DROP VIEW IF EXISTS v_MT_LD_UCC_LOCAL_CALLING_AREA;
DROP VIEW IF EXISTS v_MT_LD_UCC_NPA_NXX_LR;
DROP VIEW IF EXISTS v_MT_LD_UCC_ROUTE_NPA_NXX;
DROP VIEW IF EXISTS v_MT_LD_UCC_temp1;
DROP VIEW IF EXISTS v_MT_LD_UCC_temp2;
DROP VIEW IF EXISTS v_MT_LD_UCC_temp3;
DROP VIEW IF EXISTS v_MT_LD_UCC_temp4;
DROP VIEW IF EXISTS v_MT_LD_UCC_temp5;

CREATE VIEW IF NOT EXISTS v_MT_LD_UCC_NPA_NXX_STATE_MPS AS
SELECT 
...
CREATE VIEW IF NOT EXISTS v_MT_LD_UCC_LOCAL_CALLING_AREA AS
SELECT
DISTINCT sid, Vertical_Coordinate, Horizontal_Cordinate, 'FLCA' AS LCA_FLAG
FROM ela_v21.local_calling_area_gg
WHERE discontinue_date > from_unixtime(unix_timestamp());

CREATE VIEW IF NOT EXISTS v_MT_LD_UCC_NPA_NXX_LR AS
SELECT
DISTINCT NPA, NXX, SID
FROM ela_v21.NPA_NXX_LR_PORTED a
UNION ALL
SELECT
DISTINCT NPA, NXX, SID
FROM ela_v21.NPA_NXX_LR_GG b
WHERE b.SID <> '00000';

CREATE VIEW IF NOT EXISTS v_MT_LD_UCC_ROUTE_NPA_NXX AS
SELECT 
a.route_id, a.land_npa, a.land_nxx, b.v_coordinates, b.h_coordinates, a.switch_id
FROM
ela_v21.route_gg a
LEFT OUTER JOIN ela_v21.npa_nxx_gg b
ON a.land_npa = b.npa
AND a.land_nxx = b.nxx;

CREATE VIEW IF NOT EXISTS v_MT_LD_UCC_temp1 AS 
SELECT
recordtype,
retransmission,
sipmethod,
...
CREATE VIEW IF NOT EXISTS v_MT_LD_UCC_temp2 AS
SELECT a.*,
b.land_npa,
b.land_nxx,
b.v_coordinates,
b.h_coordinates,
SUBSTR(a.callingpartyaddress, 1, 1) AS intl_ind,
IF(SUBSTR(a.callingpartyaddress, 1, 1) = 1, SUBSTR(a.callingpartyaddress,2,3), NULL) AS CDR_NPA,
IF(SUBSTR(a.callingpartyaddress, 1, 1) = 1, SUBSTR(a.callingpartyaddress,5,3), NULL) AS CDR_NXX,
SUBSTR(a.calledpartyaddress,2,3) AS CLD_NPA,
SUBSTR(a.calledpartyaddress,5,3) AS CLD_NXX
FROM ext_cdrdm.v_MT_LD_UCC_temp1 a
LEFT OUTER JOIN ext_cdrdm.v_MT_LD_UCC_ROUTE_NPA_NXX b
ON
--Phase 2 changes starts - modified logic
...
CREATE VIEW IF NOT EXISTS v_MT_LD_UCC_temp3 AS
SELECT a.*, b.country_code
FROM ext_cdrdm.v_MT_LD_UCC_temp2 a
LEFT OUTER JOIN ext_cdrdm.v_MT_LD_UCC_NPA_NXX_STATE_MPS b
ON a.CDR_NPA = b.NPA AND a.CDR_NXX = b.NXX;

CREATE VIEW IF NOT EXISTS v_MT_LD_UCC_temp4 AS
SELECT a.*, b.sid
FROM ext_cdrdm.v_MT_LD_UCC_temp3 a
LEFT OUTER JOIN ext_cdrdm.v_MT_LD_UCC_NPA_NXX_LR b
ON a.CLD_NPA = b.NPA AND a.CLD_NXX = b.NXX;

CREATE VIEW IF NOT EXISTS v_MT_LD_UCC_temp5 AS
SELECT a.*, b.vertical_coordinate, horizontal_cordinate, lca_flag
FROM ext_cdrdm.v_MT_LD_UCC_temp4 a
LEFT OUTER JOIN ext_cdrdm.v_MT_LD_UCC_LOCAL_CALLING_AREA b
ON a.sid = b.sid AND a.V_Coordinates = b.Vertical_Coordinate AND a.H_Coordinates = b.Horizontal_Cordinate;

USE cdrdm;

DROP VIEW IF EXISTS v_MT_LD_UCC;

CREATE VIEW IF NOT EXISTS v_MT_LD_UCC AS
SELECT c.*,
(CASE WHEN c.Lst1AccessNetworkInfo = '' or c.Lst1AccessNetworkInfo IS NULL THEN 'No_cellIID'
WHEN c.callingpartyaddress = '' or c.callingpartyaddress IS NULL THEN 'No_Caller'
WHEN c.INTL_IND <> '1' THEN 'INTL'
WHEN c.country_code = 'USA' THEN 'USA'
WHEN c.LCA_FLAG IS NULL AND c.COUNTRY_CODE = 'CAN' AND (c.Lst1AccessNetworkInfo IS NOT NULL OR c.Lst1AccessNetworkInfo <> '') THEN 'LD_CAN'
WHEN c.LCA_FLAG IS NULL AND c.COUNTRY_CODE = 'USA' AND (c.Lst1AccessNetworkInfo IS NOT NULL OR c.Lst1AccessNetworkInfo <> '') THEN 'LD_US'
WHEN c.LCA_FLAG IS NOT NULL AND (c.Lst1AccessNetworkInfo IS NOT NULL OR c.Lst1AccessNetworkInfo <> '') THEN 'LOCAL'
WHEN SUBSTR(callingpartyaddress,1,2) = '18' THEN '1800XXXX'
ELSE 0 END) AS ld_ind
FROM ext_cdrdm.v_MT_LD_UCC_temp5 c;