...
--** Filename: cdrdm_DIM_CELL_SITE_INFO_LTE_FILE_SCRIPT.hql
...
--** THIS CODE LOADS LTE DIM_CELL_SITE_INFO FILES INTO TEMPORARY TABLES AND CREATES 
...
--************************************************************************************************
--************************************************************************************************
--**LOADS LTE NEW DATA INTO A TEMP TABLE
--************************************************************************************************
--************************************************************************************************
...
SELECT
*,
CONCAT(substring(${hiveconf:file_dates},1,4),'-',substring(${hiveconf:file_dates},5,2),'-',substring(${hiveconf:file_dates},7,2)) as file_date_dash,
...
--************************************************************************************************
--************************************************************************************************
--**INSERTS A NEW CGI INTO HISTORICAL TABLE FOR THE LATEST LTE RECORD WITH THE LEAST NUMBER OF NULLS.
--************************************************************************************************
--************************************************************************************************

INSERT INTO TABLE cdrdm.historical_dim_cell_site_info 
SELECT       
concat('RogersCellSiteDatabase','LTE','_',substr(a.file_date,1,4), substr(a.file_date,6,2), substr(a.file_date,9,2),'.csv') as file_name,
trim(a.site),
trim(a.cell),
--trim(a.nodeb) as nodeb,
trim(a.nodeb) as nodeb,
trim(a.cgi),
...
'LTE' as file_type, --FILE_TYPE
to_date(from_unixtime(unix_timestamp())), --INSERT_TIMESTAMP
a.file_date_dash as file_date 
from 
(
 select * from  
  (
  select *, ROW_NUMBER() OVER (PARTITION BY tmp.cgi ORDER BY tmp.row_null_cnt) as row_cnt FROM
    (
    SELECT *, if(IF(site IS NULL,1,0) + IF(cell IS NULL,1,0) +IF(nodeb IS NULL,1,0)+IF(cgi IS NULL,1,0)+IF(site_name IS NULL,1,0)+IF(original_i_s_date IS NULL,1,0)+IF(antenna_type IS NULL,1,0)+IF(azimuth IS NULL,1,0)+IF(beamwidth IS NULL,1,0)+IF(x IS NULL,1,0)+IF(y IS NULL,1,0)+IF(longitude IS NULL,1,0)+IF(latitude IS NULL,1,0)+IF(address IS NULL,1,0)+IF(city IS NULL,1,0)+IF(province IS NULL,1,0)+IF(locationcode IS NULL,1,0)+IF(file_date IS NULL,1,0) = NULL, 0, (IF(site IS NULL,1,0) + IF(cell IS NULL,1,0) +IF(nodeb IS NULL,1,0)+IF(cgi IS NULL,1,0)+IF(site_name IS NULL,1,0)+IF(original_i_s_date IS NULL,1,0)+IF(antenna_type IS NULL,1,0)+IF(azimuth IS NULL,1,0)+IF(beamwidth IS NULL,1,0)+IF(x IS NULL,1,x)+IF(y IS NULL,1,0)+IF(longitude IS NULL,1,0)+IF(latitude IS NULL,1,0)+IF(address IS NULL,1,0)+IF(city IS NULL,1,0)+IF(province IS NULL,1,0)+IF(locationcode IS NULL,1,0)+IF(file_date IS NULL,1,0))) as row_null_cnt FROM cdrdm.dim_cell_site_info_${hiveconf:site_names}_temp1
    ) as tmp 
...
--************************************************************************************************
--************************************************************************************************
--**INSERTS A CHANGED COLUMNS FOR EXISITING CGI INTO HISTORICAL TABLE FOR THE LATEST LTE RECORD WITH THE LEAST NUMBER OF NULLS.
--************************************************************************************************
--************************************************************************************************
