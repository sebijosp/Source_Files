...
--** Filename: cdrdm_DIM_CELL_SITE_INFO_UMTS_FILE_SCRIPT.hql
...
--** THIS CODE LOADS UMTS DIM_CELL_SITE_INFO FILES INTO TEMPORARY TABLES AND CREATES 
...
--************************************************************************************************
--************************************************************************************************
--**DROPS PREVIOUS LOAD OF FILES
--************************************************************************************************
--************************************************************************************************

-- DROP TABLE IF EXISTS cdrdm.dim_cell_site_info_lte_temp1;
-- DROP TABLE IF EXISTS cdrdm.dim_cell_site_info_umts_temp1;
-- DROP TABLE IF EXISTS cdrdm.dim_cell_site_info_gsm_temp1;

MSCK REPAIR TABLE ext_cdrdm.dim_cell_site_info_${hiveconf:site_names};

--************************************************************************************************
--************************************************************************************************
--**LOADS UTMS NEW DATA INTO A TEMP TABLE
--************************************************************************************************
--************************************************************************************************
...
--************************************************************************************************
--************************************************************************************************
--**INSERTS A NEW CGI INTO HISTORICAL TABLE FOR THE LATEST UTMS RECORD WITH THE LEAST NUMBER OF NULLS.
--************************************************************************************************
--************************************************************************************************
...
SELECT       
concat('RogersCellSiteDatabase','UMTS','_',substr(a.file_date,1,4), substr(a.file_date,6,2), substr(a.file_date,9,2),'.csv') as file_name,
trim(a.site),
trim(a.cell),
NULL, --nodeb,
trim(a.cgi),
CONCAT(SUBSTR(trim(a.cgi),1,LOCATE('-',trim(a.cgi))-1),CONCAT(SUBSTR(trim(a.cgi),LOCATE('-',trim(a.cgi))+1,(LOCATE('-',trim(a.cgi),second_occurrence) - LOCATE('-',trim(a.cgi)))-1),CONCAT(CONV(SUBSTR(trim(a.cgi),LOCATE('-',trim(a.cgi),second_occurrence - 1)+1,(LOCATE('-',trim(a.cgi),second_occurrence + 1) - LOCATE('-',trim(a.cgi),second_occurrence -1))-1),10,16),CONV(SUBSTR(trim(a.cgi),LOCATE('-',trim(a.cgi),last_occurrence -1)+1),10,16)))) as CGI_HEX,
trim(a.site_name),
...
NULL, --BSIC
--trim(a.primaryscramblingcode) as primaryscramblingcode,
trim(a.primaryscramblingcode) as primaryscramblingcode,
'UMTS' as file_type, --FILE_TYPE
...
(
 select * from  
  ...
  select *, ROW_NUMBER() OVER (PARTITION BY tmp.cgi ORDER BY tmp.row_null_cnt) as row_cnt FROM
    (
    SELECT *, if(IF(site IS NULL,1,0) + IF(cell IS NULL,1,0) +IF(cgi IS NULL,1,0)+IF(site_name IS NULL,1,0)+IF(original_i_s_date IS NULL,1,0)+IF(antenna_type IS NULL,1,0)+IF(azimuth IS NULL,1,0)+IF(beamwidth IS NULL,1,0)+IF(x IS NULL,1,0)+IF(y IS NULL,1,0)+IF(arfcndl IS NULL,1,0)+IF(longitude IS NULL,1,0)+IF(latitude IS NULL,1,0)+IF(address IS NULL,1,0)+IF(city IS NULL,1,0)+IF(province IS NULL,1,0)+IF(locationcode IS NULL,1,0)+IF(primaryscramblingcode IS NULL,1,0)+IF(file_date IS NULL,1,0) = NULL, 0, (IF(site IS NULL,1,0) + IF(cell IS NULL,1,0) + IF(cgi IS NULL,1,0)+IF(site_name IS NULL,1,0)+IF(original_i_s_date IS NULL,1,0)+IF(antenna_type IS NULL,1,0)+IF(azimuth IS NULL,1,0)+IF(beamwidth IS NULL,1,0)+IF(x IS NULL,1,0)+IF(y IS NULL,1,0)+IF(longitude IS NULL,1,0)+IF(latitude IS NULL,1,0)+IF(address IS NULL,1,0)+IF(city IS NULL,1,0)+IF(province IS NULL,1,0)+IF(arfcndl IS NULL,1,0)+IF(locationcode IS NULL,1,0)+IF(primaryscramblingcode IS NULL,1,0)+IF(file_date IS NULL,1,0))) as row_null_cnt FROM cdrdm.dim_cell_site_info_${hiveconf:site_names}_temp1
    ) as tmp 
...
--************************************************************************************************
--************************************************************************************************
--**INSERTS A CHANGED COLUMNS FOR EXISITING CGI INTO HISTORICAL TABLE FOR THE LATEST UTMS RECORD WITH THE LEAST NUMBER OF NULLS.
--************************************************************************************************
--************************************************************************************************
...
,a.end_date 
,a.file_date
 from cdrdm.vw_dim_cell_site_info a left join cdrdm.dim_cell_site_info_fix1 b on a.cgi_hex=b.cgi_hex and a.start_date=b.start_date where b.start_date is null;

insert into cdrdm.dim_cell_site_info select * from cdrdm.dim_cell_site_info_fix1;
