...
SRVCCIndicator,
call_timestamp_date
 FROM cdrdm.fact_gsm_cdr_stg WHERE  call_type <> 3;
...
v_les_hist_all.source_table  FROM CDRDM.LES_MAIN_FACT_GSM_STEP1 v_les_hist_all
...
SELECT DISTINCT A.first_Cell_complete_Orig,SUBSTR(A.CALL_DATE,1,10), A.CALL_TIMESTAMP_DATE
 FROM CDRDM.LES_MAIN_FACT_GSM_STEP2 A WHERE  A.first_Cell_complete_Orig IS NOT NULL AND NOT EXISTS
 ...
 FROM CDRDM.LES_MAIN_FACT_GSM_STEP2 A WHERE  A.Last_Cell_complete_Orig IS NOT NULL AND NOT EXISTS
...
truncate table CDRDM.LES_MAIN_FACT_GSM_STEP1;
truncate table CDRDM.LES_MAIN_FACT_GSM_STEP2;
