...
SELECT DISTINCT A.first_Cell_complete_Orig,SUBSTR(A.CALL_DATE,1,10), A.CALL_TIMESTAMP_DATE
 FROM CDRDM.les_main_fact_sms_cdr_step2 A WHERE  A.first_Cell_complete_Orig IS NOT NULL AND NOT EXISTS
 ...
 FROM CDRDM.les_main_fact_sms_cdr_step2 A WHERE  A.Last_Cell_complete_Orig IS NOT NULL AND NOT EXISTS
