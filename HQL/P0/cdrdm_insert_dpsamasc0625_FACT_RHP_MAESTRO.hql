...
NULL,
ic_inc_prefix,
case when (length(trim(carrier_connect_date) ) > 0) then cast (CONCAT(substr(cast(year(from_unixtime(unix_timestamp())) as char(4)),1,3),substr (carrier_connect_date,1,1),'-',substr (carrier_connect_date,2,2),'-',substr (carrier_connect_date,4,2),' ',substr (carrier_connect_time,1,2),':',substr (carrier_connect_time,3,2),':',substr (carrier_connect_time,5,2)) as STRING) end as carrier_connect_datetime,
cast(carrier_Elapsed_Time as char(9)), --cast(int)
...
CONCAT(SUBSTR(CONCAT('2',(substr(('0200000'),1,7 - length(trim(cast(field_date as char(5)))))),trim(cast(field_date as char(5)))),1,4),'-',SUBSTR(CONCAT('2',(substr(('0200000'),1,7 - length(trim (cast(field_date as char(5)))))),trim(cast(field_date as char(5)))),5,2),'-',SUBSTR(CONCAT('2',(substr(('0200000'),1,7 - length(trim (cast(field_date as char(5)))))),trim(cast(field_date as char(5)))),7,2)) as field_date
FROM cdrdm.dpsamasc0625 a ${hiveconf:WHERE_clause}
AND a.file_name like '%_mst.dat' ;
