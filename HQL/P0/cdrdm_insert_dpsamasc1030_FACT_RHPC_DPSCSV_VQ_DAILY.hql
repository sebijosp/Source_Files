...
AND (a.file_name not like '%_mst.dat' AND a.file_name not like '%_sf.dat' AND a.file_name not like 'dpsama_csv_dps99%') ; 
...
FROM cdrdm.dpsamasc1030 a ${hiveconf:WHERE_clause}
AND
((CONCAT(cast(a.activating_NPA as char(3)),cast(a.activating_Number as char(7))) IN (SELECT b.subscriber_no from ela_v21.subscriber b))
...
alter table cdrdm.FACT_RHPC_DPSCSV_VQ_DAILY drop if exists partition(field_date<'2019-02-01');

alter table cdrdm.FACT_RHP_MANUAL drop if exists partition(field_date<'2019-02-01');

alter table cdrdm.FACT_RHP_MAESTRO drop if exists partition(field_date<'2019-02-01');
