...
FaultCode,
EOSInfo,
CASE WHEN TRIM(disconnectingparty) = '' THEN NULL ELSE disconnectingparty END AS disconnectingparty,
internalCauseAndLoc, 
trafficactivitycode,
ChargedParty,
...
--Data Mart
(CASE
WHEN RECORD_TYPE = 'transit' THEN (CASE WHEN cleansedcharged = '' THEN (CASE WHEN CleansedCalling = '' THEN (CASE WHEN cleansedcalled = '' THEN -recordsequencenumber ELSE CleansedCalled END) 
                  ELSE cleansedcalling END) ELSE cleansedCharged END)
WHEN RECORD_TYPE = 'mSOriginating' THEN CASE  WHEN cleansedCharged = '' THEN CASE WHEN CleansedCalling = '' THEN recordsequencenumber ELSE CleansedCalling END ELSE cleansedCharged END
WHEN RECORD_TYPE = 'roamingCallForwarding' THEN CASE  WHEN cleansedCharged = '' THEN CASE WHEN CleansedCalled = '' THEN -recordsequencenumber ELSE CleansedCalled END ELSE cleansedCharged  END
WHEN RECORD_TYPE = 'callForwarding' THEN CASE  WHEN CleansedRedirecting = '' THEN CASE WHEN cleansedCharged = '' THEN -recordsequencenumber ELSE cleansedCharged END ELSE COALESCE(CleansedRedirecting,0) END
WHEN RECORD_TYPE = 'mSOriginatingSMSinMSC' THEN CASE  WHEN CleansedCalling = '' THEN  -recordsequencenumber ELSE CleansedCalling END
WHEN RECORD_TYPE IN ('mSTerminating','mSTerminatingSMSinMSC') THEN CASE
                        WHEN CleansedCalled = 0 OR CleansedCalled = '' THEN CASE WHEN CleansedRedirecting IS NULL OR CleansedRedirecting = '' THEN  
