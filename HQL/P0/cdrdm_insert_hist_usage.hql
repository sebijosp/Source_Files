...
--0.3 - saseenthar - 6/10/2016 - Updated ALS_IND derivation logic as part of UCC Phase 2 Requirement
...
--Phase 2 changes starts  
      ...
      SELECT subscriber_no, customer_BAN, ALS_IND, EFFECTIVE_DATE, SYS_UPDATE_DATE	  
...
SELECT subscriber_no, BAN, feature_code AS ALS_IND, 3 AS tbl_priority FROM (
  ...
  (
--Phase 2 changes starts  
  SELECT sfh.subscriber_no, sfh.BAN, sfh.feature_code, sfh.FTR_EFFECTIVE_DATE, sfh.FTR_EXPIRATION_DATE, sfh.SYS_CREATION_DATE
  FROM
  (
        SELECT subscriber_no, BAN, feature_code, FTR_EFFECTIVE_DATE, FTR_EXPIRATION_DATE, SYS_CREATION_DATE
        FROM ELA_V21.srv_ftr_history z
        WHERE z.subscriber_no IS NOT NULL 
        AND z.BAN IS NOT NULL) sfh
        --AND feature_code = 'UCCUSR'    
        --AND z.feature_code IN 
       LEFT SEMI JOIN
       (Select a.feature_code from ELA_V21.feature_gg a where a.switch_code in (Select b.SW_Code from cdrdm.dim_Sw_codes b where b.pick = 'Y')) fgg
       on
       sfh.feature_code = fgg.feature_code
--Phase 2 changes ends
   ) subscr3
...
SELECT subscriber_no, BAN, feature_code AS ALS_IND, 4 AS tbl_priority FROM (
  ...
  FROM ( SELECT subscriber_no, BAN, channel_seizure_dt FROM ods_usage.usage ) usg
  JOIN (
  SELECT sf.subscriber_no, sf.BAN, sf.feature_code, sf.FTR_EFFECTIVE_DATE, sf.FTR_EXPIRATION_DATE, sf.SYS_CREATION_DATE
  FROM
  (
        SELECT subscriber_no, BAN, feature_code, FTR_EFFECTIVE_DATE, FTR_EXPIRATION_DATE, SYS_CREATION_DATE
        FROM ELA_V21.service_feature z
        WHERE subscriber_no IS NOT NULL
        AND BAN IS NOT NULL ) sf
--Phase 2 changes starts
        --AND feature_code = 'UCCUSR'    
        --AND z.feature_code IN 
        LEFT SEMI JOIN 
        (Select a.feature_code from ELA_V21.feature_gg a where a.switch_code in (Select b.SW_Code from cdrdm.dim_Sw_codes b where b.pick = 'Y')) GG
        ON
        SF.feature_code = gg.feature_code
--Phase 2 changes ends		
   ) subscr4
...
LEFT OUTER JOIN (
      SELECT subscriber_no, BAN, ACCOUNT_SUB_TYPE,COMPANY_CODE, FRANCHISE_CD, TR_BA_ACCOUNT_TYPE, PRODUCT_TYPE
...
AND usg.BAN = mps.BAN
--Phase 2 changes starts
AND usg.PRODUCT_TYPE = mps.PRODUCT_TYPE
--Phase 2 changes ends
