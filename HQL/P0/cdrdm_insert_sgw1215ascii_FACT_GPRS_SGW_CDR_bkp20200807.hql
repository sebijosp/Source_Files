...
APN,
Served_IMEI,
if(userLocationInformation1 is not NULL and length(trim(userLocationInformation1)) > 0 and trim(substr(userLocationInformation1,12,7)) is not NULL,concat(trim(substr(userLocationInformation1,1,3)),trim(substr(userLocationInformation1,4,3)),trim(substr(userLocationInformation1,7,5)),conv(concat(conv(trim(substring(userLocationInformation1,12,7)),10,16),'0',trim(substring(userLocationInformation1,19,5))),16,10)),NULL) as ECID,
if(userLocationInformation1 is not NULL and length(trim(userLocationInformation1)) > 0 and trim(substr(userLocationInformation1,7,5)) is not NULL,conv(trim(substr(userLocationInformation1,7,5)),10,16),NULL) as TAC_HEX,
if(userLocationInformation1 is not NULL and length(trim(userLocationInformation1)) > 0 and trim(substr(userLocationInformation1,12,7)) is not NULL,lpad(concat(conv(trim(substr(userLocationInformation1,12,7)),10,16),'0',trim(substr(userLocationInformation1,19,5))),8,'0'),NULL) as ECID_HEX,
file_name,
record_start,
record_end,
record_type,
