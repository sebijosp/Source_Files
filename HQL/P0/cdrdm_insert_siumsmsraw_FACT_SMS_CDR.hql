...
CONCAT(substr(local_subscriber_date,1,4),'-',substr(local_subscriber_date,6,2),'-',substr(local_subscriber_date,9,2),' ',SUBSTR(local_subscriber_time,1,8)) as local_subscriber_timestamp, -- STRING
-- UPDATED
...
CONCAT(substr(sms_date,1,4),'-',substr(sms_date,6,2),'-',substr(sms_date,9,2),' ',SUBSTR(sms_time,1,8)) as date_1, -- check SMS_TIME value / 'YYYY-MM-DD HH24:MI:SS'
...
INSERT INTO TABLE cdrdm.fact_sms_cdr_stg
SELECT
a.subscriber_no,
a.event_record_id,
a.spid,
