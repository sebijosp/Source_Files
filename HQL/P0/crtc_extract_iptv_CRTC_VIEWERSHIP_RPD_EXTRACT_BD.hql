...
!echo Creating table if not exists :: CRTC_VIEWERSHIP.RPD_EXTRACT_BD;

CREATE TABLE if not exists CRTC_VIEWERSHIP.RPD_EXTRACT_BD
(
CAN VARCHAR(20),
...
!echo loading iptv Linear data load on table :: CRTC_VIEWERSHIP.RPD_EXTRACT_BD;

Insert INTO TABLE CRTC_VIEWERSHIP.RPD_EXTRACT_BD
...
from IPTV.IP_PLAYBACK_SUMFCT A join (SELECT DISTINCT CAN FROM CRTC_VIEWERSHIP.SAF_EXTRACT_BD) B on trim(A.rogers_account_id)=trim(B.CAN)
...
!echo loading VOD data load on table :: CRTC_VIEWERSHIP.RPD_EXTRACT_BD;
...
!echo loading CDVR data load on table :: CRTC_VIEWERSHIP.RPD_EXTRACT_BD;
...
FROM
CRTC_VIEWERSHIP.RPD_EXTRACT_BD
