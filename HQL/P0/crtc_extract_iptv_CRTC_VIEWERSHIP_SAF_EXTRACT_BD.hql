...
!echo dropping table if exists :: CRTC_VIEWERSHIP.IPTV_SAF_EXTRACT_STG_BD;

drop table if exists CRTC_VIEWERSHIP.IPTV_SAF_EXTRACT_STG_BD;

!echo Creating table if not exists :: CRTC_VIEWERSHIP.IPTV_SAF_EXTRACT_STG_BD;

CREATE TABLE if not exists CRTC_VIEWERSHIP.IPTV_SAF_EXTRACT_STG_BD
(
CAN  VARCHAR(50),
PACKAGE VARCHAR(50),
...
!echo loading load on table :: CRTC_VIEWERSHIP.IPTV_SAF_EXTRACT_STG_BD; 
insert into CRTC_VIEWERSHIP.IPTV_SAF_EXTRACT_STG_BD
...
from app_maestro.hist_pntrdly A join
...
!echo loading iptv data on table :: CRTC_VIEWERSHIP.SAF_EXTRACT_STG_BD;

Insert INTO TABLE CRTC_VIEWERSHIP.SAF_EXTRACT_STG_BD
...
FROM
CRTC_VIEWERSHIP.IPTV_SAF_EXTRACT_STG_BD)DUP
where DUP.fsa_removal=1 and DUP.spec_removal=1;

!echo dropping table if exists :: CRTC_VIEWERSHIP.SAF_EXTRACT_BD;

drop table if exists CRTC_VIEWERSHIP.SAF_EXTRACT_BD;

!echo Creating table the CRTC_VIEWERSHIP.SAF_EXTRACT_BD table which will contain Both Legacy_Cable & IPTV Customers : CRTC_VIEWERSHIP.SAF_EXTRACT_BD;

CREATE TABLE if not exists CRTC_VIEWERSHIP.SAF_EXTRACT_BD
(
CAN  VARCHAR(50),
PACKAGE VARCHAR(50),
TOTALBOXES VARCHAR(25),
...
!echo loading final table with FSA_CODE more than 100 on table :: CRTC_VIEWERSHIP.SAF_EXTRACT_BD;

Insert INTO TABLE CRTC_VIEWERSHIP.SAF_EXTRACT_BD
SELECT
CAN,
PACKAGE,
TOTALBOXES,
...
DEVICE_ID
FROM
CRTC_VIEWERSHIP.SAF_EXTRACT_STG_BD
where FSA_CODE IN (SELECT FSA_CODE FROM CRTC_VIEWERSHIP.SAF_EXTRACT_STG_BD group by FSA_CODE  having count(*)>100);
