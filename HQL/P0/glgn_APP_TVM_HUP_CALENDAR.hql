...
drop table if exists ext_APP_TVM.HUP_CALENDAR;
CREATE table ext_APP_TVM.HUP_CALENDAR stored as orc
AS SELECT BAN,
    SUBSCRIBER_NO,
    LST_FRANCHISE_TP,
    LST_BILL_CYCLE,
...
A.VOICE_BUNDLE,
A.VOICE_BUCKET,
ROW_NUMBER () OVER (PARTITION BY A.BAN,A.SUBSCRIBER_NO,A.HUP_YEAR,A.HUP_MONTH ORDER BY A.MSP_YEAR DESC, A.MSP_MONTH DESC,B.MSP_YEAR DESC, B.MSP_MONTH DESC ) SEQ ,
CASE
  WHEN B.BAN IS NOT NULL
  THEN 1
  ELSE 0
END AS HUP_AND_PPC  FROM
(SELECT * FROM app_tvm.HUP 
WHERE CONCAT(MSP_YEAR,'-',LPAD(MSP_MONTH,2,'0'),'-','01') BETWEEN add_months(CONCAT(HUP_YEAR,'-',LPAD(HUP_MONTH,2,'0'),'-','01'),-1) AND add_months(CONCAT(HUP_YEAR,'-',LPAD(HUP_MONTH,2,'0'),'-','01'),+1) and HUP_STATUS_DATE < date_add(last_day(current_date),1) ) A LEFT JOIN (select * from app_tvm.ppc
...
) B ON A.BAN= B.BAN AND A.SUBSCRIBER_NO = B.SUBSCRIBER_NO AND A.HUP_YEAR= B.PPC_YEAR AND A.HUP_MONTH= B.PPC_MONTH) y WHERE SEQ = 1;
drop table if exists APP_TVM.HUP_CALENDAR;
alter table ext_APP_TVM.HUP_CALENDAR rename to APP_TVM.HUP_CALENDAR;
