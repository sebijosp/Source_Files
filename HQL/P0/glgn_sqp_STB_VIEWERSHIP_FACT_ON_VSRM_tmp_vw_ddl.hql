create or replace view ext_stb_viewership.viewership_fact_on_vsrm_sqp_imp as
select
...
CAST('VSRM' AS VARCHAR(10)) as SOURCE_TYPE,
CAST(split(INPUT__FILE__NAME, '/userapps/glagoon/stb_viewership/ON/vsrm/landing/')[1] as varchar(100)) as SOURCE_FILENAME,
...
FROM ext_stb_viewership.viewership_fact_on_vsrm_src_file
where CAST(if(LOWER(START_TIME) = 'null', null, from_unixtime(unix_timestamp(start_time, 'MM/dd/yyyy HH:mm:ss'))) AS TIMESTAMP) is not null
