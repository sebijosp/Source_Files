CREATE OR REPLACE VIEW `iptv.guide_start_error_fct_incr_vw` AS WITH REF_TBL AS (
SELECT
...
AND   `ref_tbl`.`iptv_source_table`   = 'IPTV.GUIDE_START_ERROR_FCT'
),
BDM_CONTROL AS (
SELECT
...
FROM RAW_FILTERED R
INNER JOIN `IPTV`.`GUIDE_START_ERROR_FCT` `F`
