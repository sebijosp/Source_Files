INSERT INTO HEM.CM_THROUGHPUT_STG_1 PARTITION(EVENT_DATE_UTC)
...
SELECT
  ...
  nvl(Prod.TIER,ptr.TIER) TIER,
  nvl(nvl(Prod.DOWNLOAD_SPEED_KBPS,ptr.DOWNLOAD_SPEED_KBPS),100000) DOWNLOAD_SPEED_KBPS,
  nvl(nvl(Prod.UPLOAD_SPEED_KBPS,ptr.UPLOAD_SPEED_KBPS),1000) UPLOAD_SPEED_KBPS,
  Prod.TECHNOLOGY,
  Prod.DS_TUNERS_COUNT,
  Prod.US_TUNERS_COUNT,
  CASE WHEN (nvl(nvl(Prod.DOWNLOAD_SPEED_KBPS,ptr.DOWNLOAD_SPEED_KBPS),100000) / 1000) = 1000 then 940 else (nvl(nvl(Prod.DOWNLOAD_SPEED_KBPS,ptr.DOWNLOAD_SPEED_KBPS),100000) / 1000) END DS_SPEED,
  CASE WHEN (nvl(nvl(Prod.UPLOAD_SPEED_KBPS,ptr.UPLOAD_SPEED_KBPS),1000) / 1000) = 1000 then 940 else (nvl(nvl(Prod.UPLOAD_SPEED_KBPS,ptr.UPLOAD_SPEED_KBPS),1000) / 1000) END US_SPEED,
  CASE WHEN Prod.TECHNOLOGY = "D3.1" THEN 'Y' 
       WHEN (nvl(nvl(Prod.DOWNLOAD_SPEED_KBPS,ptr.DOWNLOAD_SPEED_KBPS),100000) / 1000) = 1000 AND Prod.TECHNOLOGY <> "D3.1" THEN "N"
       ELSE "Y"
       END REPORT_FLAG,
...
LEFT JOIN HEM.PRODUCT_TIER_REFERENCE ptr
  on ptr.PRODUCT_CODE = newmacs.PRODUCT_CODE
WHERE 
