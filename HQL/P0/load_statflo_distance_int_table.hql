...
INSERT OVERWRITE TABLE NW_TOWER_USAGE.STATFLO_DISTANCE_TEMP
SELECT A.ECID,
  A.BAN,
  A.SUBSCRIBER_NO,
  A.LST_FRANCHISE_TP,
...
JOIN (SELECT NVL(C.SUBSCRIBER_NO,D.SUBSCRIBER_NO) AS SUBSCRIBER_NO,NVL(C.Y,D.Y) AS Y,NVL(C.X,D.X) X
      FROM NW_TOWER_USAGE.STATFLO_HOME_TOWER_TEMP C
...
INSERT OVERWRITE TABLE NW_TOWER_USAGE.STATFLO_DISTANCE_INT
SELECT STORE_ID,BANNER,LOCATION_NAME,CMA,ECID,
  BAN,
  SUBSCRIBER_NO,
  DISTANCECALC,
  CHAIN_CODE,
  ...
  LOCATION_TYPE,
  SQ_FT
FROM NW_TOWER_USAGE.STATFLO_DISTANCE_TEMP BASE
JOIN
  (SELECT B.STORE_ID,
    B.CHAIN_CODE,
    B.LOCATION_ID,
  ...
  FROM
    (SELECT CTN_LAT,
      CTN_LON
    FROM NW_TOWER_USAGE.STATFLO_DISTANCE_TEMP
    GROUP BY CTN_LAT,
      CTN_LON
    )A
  JOIN
