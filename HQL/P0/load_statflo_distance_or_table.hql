...
INSERT OVERWRITE TABLE NW_TOWER_USAGE.STATFLO_DISTANCE_TEMP_OR
SELECT DISTANCE.ECID,DISTANCE.BAN,DISTANCE.SUBSCRIBER_NO,DISTANCE.LST_FRANCHISE_TP,DISTANCE.CTN_LAT,DISTANCE.CTN_LON
FROM
  (SELECT A.ECID,
    A.BAN,
    A.SUBSCRIBER_NO,
  ...
  JOIN (SELECT NVL(C.SUBSCRIBER_NO,D.SUBSCRIBER_NO) AS SUBSCRIBER_NO,NVL(C.Y,D.Y) AS Y,NVL(C.X,D.X) X
      FROM (SELECT SUBSCRIBER_NO,X,Y FROM NW_TOWER_USAGE.STATFLO_HOME_TOWER_TEMP 
...
SELECT STORE_ID,
  ...
  LOCATION_TYPE,
  SQ_FT
FROM NW_TOWER_USAGE.STATFLO_DISTANCE_TEMP_OR BASE
JOIN
  (SELECT B.STORE_ID,
    B.CHAIN_CODE,
    B.LOCATION_ID,
  ...
  FROM
    (SELECT CTN_LAT,
      CTN_LON
    FROM NW_TOWER_USAGE.STATFLO_DISTANCE_TEMP_OR
    GROUP BY CTN_LAT,
      CTN_LON
    )A
  JOIN
    (SELECT BRSC_ID     AS STORE_ID,
      CHAINOFSTORESCODE AS CHAIN_CODE,
      LOCATION_ID,
      CASE
        WHEN FRANCHISE = 'R'
        THEN 'Rogers'
...
AND CASE WHEN LST_FRANCHISE_TP = 'R' THEN 'Rogers' WHEN LST_FRANCHISE_TP = 'F' THEN 'Fido' END = BANNER;
