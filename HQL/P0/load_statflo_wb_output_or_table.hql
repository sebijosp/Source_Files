...
INSERT OVERWRITE TABLE NW_TOWER_USAGE.STATFLO_WB_OUTPUT_OR_TEMP
...
FROM (SELECT DISTANCE.*, ROW_NUMBER() OVER (PARTITION BY DISTANCE.SUBSCRIBER_NO ORDER BY DISTANCE.DISTANCECALC) AS STORE_DISTANCE_RNK
      FROM NW_TOWER_USAGE.STATFLO_WB_DISTANCE_OR DISTANCE ) QRY
WHERE STORE_DISTANCE_RNK = 1;

INSERT OVERWRITE TABLE NW_TOWER_USAGE.STATFLO_WB_OUTPUT_OR
SELECT A.STORE_ID,
  A.CHAIN_CODE,
  A.LOCATION_ID,
  A.BANNER,
  ...
  A.LOCATION_TYPE,
  A.SQ_FT
FROM NW_TOWER_USAGE.STATFLO_WB_OUTPUT_OR_TEMP A
JOIN
  (SELECT STATFLO.*
  FROM NW_TOWER_USAGE.STATFLO_WB_OUTPUT_OR_TEMP STATFLO
  JOIN ODS.BAN_PRIMARY_SUBSCRIBER BPS
  ON STATFLO.SUBSCRIBER_NO = BPS.SUBSCRIBER_NO
  AND STATFLO.BAN          = BPS.BAN
  ) B ON A.BAN             = B.BAN
AND A.STORE_ID             = B.STORE_ID
LEFT JOIN (SELECT BAN FROM NW_TOWER_USAGE.STATFLO_WB_OUTPUT_AND GROUP BY BAN) C
ON A.BAN     = C.BAN
WHERE C.BAN IS NULL;
