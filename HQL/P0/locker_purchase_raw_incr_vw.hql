CREATE OR REPLACE VIEW `iptv`.`locker_purchase_raw_incr_vw` AS WITH REF_TBL AS (
SELECT
...
AND   `ref_tbl`.`iptv_source_table`   = 'IPTV.LOCKER_PURCHASE_RAW'
),
BDM_CONTROL AS (
SELECT
...
`header`.`money`.`starttime` `money_starttime`,
...
`assetright`.`assetid`,
`assetright`.`title`,
`assetright`.`starttime` `assetright_starttime`,
`assetright`.`endtime`,
`assetright`.`purchasecategory`,
`hdp_file_name`,
`hdp_create_ts`,
`hdp_update_ts`,
`received_date`
from `iptv`.`locker_purchase_raw` `R`
