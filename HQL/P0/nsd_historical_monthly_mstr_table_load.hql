...
drop table if exists ext_cdrdm.cdr_sub_level_hme_twr_yearly_hist;
create table if not exists ext_cdrdm.cdr_sub_level_hme_twr_yearly_hist stored as orc as
select * from (
select *, ROW_NUMBER() OVER(PARTITION BY subscriber_no ORDER BY hme_tower DESC) as home_tower from (
select
subscriber_no, 
...
drop table ext_cdrdm.cdr_cstmr_ltr_raw_monthly_hist;
create table ext_cdrdm.cdr_cstmr_ltr_raw_monthly_hist as 
select
distinct
customer_id,
nps_score,
...
drop table ext_cdrdm.cdr_cstmr_ltr_ecid_monthly_hist;
create table ext_cdrdm.cdr_cstmr_ltr_ecid_monthly_hist as 
select
qry.*,
hme_twr.locationco,
ecid_sub_map.ecid
...
from ext_cdrdm.cdr_cstmr_ltr_raw_monthly_hist a
left join
...
left join
ext_cdrdm.cdr_sub_level_hme_twr_yearly_hist hme_twr
...
LEFT JOIN 
(select ecid, avg(nps_score) as avg_nps from ext_cdrdm.cdr_cstmr_ltr_ecid_monthly_hist
group by ecid) ltr
...
left join 
(select locationco, count(*) as aggr_nps_surveys from ext_cdrdm.cdr_cstmr_ltr_ecid_monthly_hist group by locationco) ltr
...
drop table if exists ext_cdrdm.sub_hme_twr_port_out_monthly_hist;
drop table if exists ext_cdrdm.port_out_monthly_rolledup_hist;
drop table if exists ext_cdrdm.cdr_cstmr_ltr_raw_monthly_hist;
drop table if exists ext_cdrdm.cdr_cstmr_ltr_ecid_monthly_hist;
drop table if exists ext_cdrdm.cdr_sub_level_hme_twr_yearly_hist;
