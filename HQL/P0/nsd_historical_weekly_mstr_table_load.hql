...
drop table if exists ext_cdrdm.cdr_dim_cell_site_info_latest_hist;
!echo "Commencing temp table ext_cdrdm.cdr_dim_cell_site_info_latest_hist creation"; 
create table if not exists ext_cdrdm.cdr_dim_cell_site_info_latest_hist stored as orc as
...
!echo "Temp table ext_cdrdm.cdr_dim_cell_site_info_latest_hist creation successful";
--************************************************************************************************************
-- ***** Calculation of Home Tower - Run Weekly (with record_opening_date of 3 months back).
--************************************************************************************************************


drop table if exists ext_cdrdm.cdr_sub_level_hme_twr_hist;
!echo "Commencing temp table ext_cdrdm.cdr_sub_level_hme_twr_hist creation";
create table if not exists ext_cdrdm.cdr_sub_level_hme_twr_hist stored as orc as
select * from (
...
INNER JOIN ext_cdrdm.cdr_dim_cell_site_info_latest_hist as bb
...
;
!echo "Temp table ext_cdrdm.cdr_sub_level_hme_twr_hist creation successful";

SET hive.execution.engine=mr;
drop table if exists ext_cdrdm.cdr_sub_level_hme_twr_year_hist;
create table if not exists ext_cdrdm.cdr_sub_level_hme_twr_year_hist stored as orc as
select * from (
select *, ROW_NUMBER() OVER(PARTITION BY subscriber_no ORDER BY hme_tower DESC) as home_tower from (
select
subscriber_no, 
...
--************************************************************************************************************
SET hive.execution.engine=tez;	
drop table if exists ext_cdrdm.cdr_base_cstmr_usge_orc_hist;
!echo "Commencing temp table ext_cdrdm.cdr_base_cstmr_usge_orc_hist creation";
create table if not exists ext_cdrdm.cdr_base_cstmr_usge_orc_hist stored as orc as
select 
...
plmn_id,
tracking_area_code
) as aa  LEFT JOIN ext_cdrdm.cdr_dim_cell_site_info_latest_hist as bb ON concat(substr(aa.plmn_id,0,3),'-',substr(aa.plmn_id,-3),'-',conv(aa.tracking_area_code,16,10),'-',conv(aa.eutran_cellid,16,10)) = trim(bb.cgi)
...
;
SET hive.execution.engine=mr;
Insert into ext_cdrdm.cdr_base_cstmr_usge_orc_hist
...
) as cc LEFT JOIN ext_cdrdm.cdr_dim_cell_site_info_latest_hist as dd ON trim(cc.first_cell_id) = trim(dd.cgi_hex)
;
Insert into ext_cdrdm.cdr_base_cstmr_usge_orc_hist
...
and IMSI >= 302370000000000 and IMSI <=302720999999999
) as ee LEFT JOIN ext_cdrdm.cdr_dim_cell_site_info_latest_hist as ff ON trim(ee.new_node) = trim(ff.cgi_hex)
;

!echo "Temp table ext_cdrdm.cdr_base_cstmr_usge_orc_hist load successful";
--************************************************************************************************************
--***** Count number of customers per tower and per weekend.
--************************************************************************************************************
SET hive.execution.engine=tez;
drop table if exists ext_cdrdm.cdr_base_cstmr_usge_sub_cnt_rolledup_hist;
!echo "Commencing temp table ext_cdrdm.cdr_base_cstmr_usge_sub_cnt_rolledup_hist creation";
create table if not exists ext_cdrdm.cdr_base_cstmr_usge_sub_cnt_rolledup_hist stored as orc as
select 
...
from ext_cdrdm.cdr_base_cstmr_usge_orc_hist
...
!echo "Temp table ext_cdrdm.cdr_base_cstmr_usge_sub_cnt_rolledup_hist creation successful";

--************************************************************************************************************
--*****Join Customer Data with Home Tower Data and pick up ecid from hash_lookup.ecid_uuid_ban_can
--************************************************************************************************************

drop table if exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_hist;
!echo "Commencing temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_hist creation";
create table if not exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_hist stored as orc as
...
(select * from ext_cdrdm.cdr_base_cstmr_usge_orc_hist where subscriber_no is not NULL) as customer
LEFT JOIN (select * from ext_cdrdm.cdr_sub_level_hme_twr_hist where subscriber_no is not NULL and locationco is not NULL) as hme_twr 
...
!echo "Temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_hist creation successful";
--************************************************************************************************************
--*****Pull NPS raw data from nps_raw.NPS_CONSOLIDATED
--************************************************************************************************************

drop table ext_cdrdm.cdr_cstmr_ltr_raw_hist;
create table ext_cdrdm.cdr_cstmr_ltr_raw_hist as 
select
distinct
customer_id,
nps_score,
...
drop table ext_cdrdm.cdr_cstmr_ltr_ecid_hist;
create table ext_cdrdm.cdr_cstmr_ltr_ecid_hist as 
select
qry.*,
hme_twr.locationco,
ecid_sub_map.ecid
...
from ext_cdrdm.cdr_cstmr_ltr_raw_hist a
left join
...
left join
ext_cdrdm.cdr_sub_level_hme_twr_year_hist hme_twr
...
--************************************************************************************************************
--*****Join cdr_cstmr_hme_twr_ecid_hist with UCAR (on ecid) to get msf and nps
--************************************************************************************************************
SET hive.execution.engine=mr;

drop table if exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf1_hist;
!echo "Commencing temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf1_hist creation";
create table if not exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf1_hist stored as orc as
select customer.*,
case when date_format(record_opening_date,'u') = 7 then record_opening_date else next_day(record_opening_date,"Sunday") end as week_ending,
...
ltr.avg_nps
from 
(select * from ext_cdrdm.cdr_cstmr_hme_twr_ecid_hist) as customer
...
LEFT JOIN 
(select ecid, avg(nps_score) as avg_nps from ext_cdrdm.cdr_cstmr_ltr_ecid_hist
group by ecid) ltr
ON customer.ecid = ltr.ecid;

!echo "Temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf1_hist creation successful";
--************************************************************************************************************
--*****Join cdr_cstmr_hme_twr_ecid_hist with UCAR (on ecid) to get msf and nps
--************************************************************************************************************
SET hive.execution.engine=tez;

drop table if exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_hist;
!echo "Commencing temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_hist creation";
create table if not exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_hist stored as orc as
select customer.*,
churn.model_score as model_score
from 
(select * from ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf1_hist) as customer
...
;
!echo "Temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_hist creation successful";
...
drop table if exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_hist;
!echo "Commencing temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_hist creation";
create table if not exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_hist stored as orc AS
SELECT
week_ending,
ecid,
sum(duration) as mou,
...
FROM ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_hist
GROUP BY week_ending, ecid;
!echo "Temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_hist creation successful";
--************************************************************************************************************
...
drop table if exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_hist;
!echo "Commencing temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_hist creation";
create table if not exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_hist stored as orc  AS
SELECT
  a.ecid as ecid,
...
INNER JOIN
  ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_hist b
  ON a.ecid = b.ecid and a.week_ending = b.week_ending;  

!echo "Temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_hist creation successful"; 
--************************************************************************************************************
--Create Rolled Up version of cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_hist
--************************************************************************************************************
drop table if exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup_hist;
!echo "Commencing temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup_hist creation";
create table if not exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup_hist stored as orc as
select 
qry.*,
...
from ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_hist
group by locationco, week_ending) qry
left join 
(select locationco, count(*) as aggr_nps_surveys from ext_cdrdm.cdr_cstmr_ltr_ecid_hist group by locationco) ltr
on qry.locationco = ltr.locationco;

!echo "Temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup_hist creation successful";		

--------------------------------------------------------------------------------------------------------------
--************************************************************************************************************
--Creation of temp table for count/percentage of promoters and detractors 
--************************************************************************************************************
drop table if exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup1_hist;
!echo "Commencing temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup1_hist creation";
create table if not exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup1_hist stored as orc as
select 
week_ending,
locationco, 
...
!echo "Temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup1_hist creation successful";	   
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- 
--************************************************************************************************************
-- Set of Temp tables to calculate below fields for weekly master:
--  1. Port Out
...
drop table if exists ext_cdrdm.port_out_sub_list_hist;
!echo "Commencing temp table ext_cdrdm.port_out_sub_list_hist creation";
create table if not exists ext_cdrdm.port_out_sub_list_hist stored as orc as
select
...
!echo "Temp table ext_cdrdm.port_out_sub_list_hist creation successful";
--************************************************************************************************************
-- get the home towers of the subscribers who ported out - weekly run
--************************************************************************************************************

drop table if exists ext_cdrdm.sub_hme_twr_port_out_hist;
!echo "Commencing temp table ext_cdrdm.sub_hme_twr_port_out_hist creation";
create table if not exists ext_cdrdm.sub_hme_twr_port_out_hist stored as orc as
select * from (
select a.*, 
b.locationco,
b.site_name,
...
b.hme_tower,
b.home_tower
from ext_cdrdm.port_out_sub_list_hist as a 
LEFT JOIN  ext_cdrdm.cdr_sub_level_hme_twr_hist as b 
...
!echo "Temp table ext_cdrdm.sub_hme_twr_port_out_hist creation successful";
--************************************************************************************************************
-- roll up counts of port outs on week_end + tower_id level - weekly run
--************************************************************************************************************

drop table if exists ext_cdrdm.port_out_weekly_rolledup_hist;
!echo "Commencing temp table ext_cdrdm.port_out_weekly_rolledup_hist creation";
create table if not exists ext_cdrdm.port_out_weekly_rolledup_hist stored as orc as
select week_ending, locationco,sum(churn_ind) as cnt_total
from ext_cdrdm.sub_hme_twr_port_out_hist
group by week_ending, locationco;

!echo "Temp table ext_cdrdm.port_out_weekly_rolledup_hist creation successful";
--************************************************************************************************************
...
SELECT 
  ...
  LEFT JOIN ext_cdrdm.port_out_weekly_rolledup_hist po
  on mref.week_end_date = po.week_ending and mref.tower_id = po.locationco
  LEFT JOIN ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup_hist ct
  on mref.week_end_date = ct.week_ending and mref.tower_id = ct.locationco
  LEFT JOIN ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup1_hist ltr
  on mref.week_end_date = ltr.week_ending and mref.tower_id = ltr.locationco
  LEFT JOIN ext_cdrdm.cdr_base_cstmr_usge_sub_cnt_rolledup_hist scn
...
drop table  if exists  ext_cdrdm.sub_hme_twr_port_out_hist;
drop table  if exists  ext_cdrdm.port_out_weekly_rolledup_hist;
drop table if exists ext_cdrdm.cdr_cstmr_ltr_raw_hist;
drop table if exists ext_cdrdm.cdr_cstmr_ltr_ecid_hist;
drop table if exists ext_cdrdm.cdr_sub_level_hme_twr_year_hist;
