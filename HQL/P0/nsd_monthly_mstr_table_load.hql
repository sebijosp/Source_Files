...
drop table if exists ext_cdrdm.cdr_dim_cell_site_info_latest_monthly;
!echo "Commencing temp table cdr_dim_cell_site_info_latest_monthly creation";
create table if not exists ext_cdrdm.cdr_dim_cell_site_info_latest_monthly stored as orc as
...
!echo "Temp table cdr_dim_cell_site_info_latest_monthly creation successful";
--************************************************************************************************************
-- ***** Calculation of Home Tower - Run Monthly (with record_opening_date of 3 months back).
--************************************************************************************************************


drop table if exists ext_cdrdm.cdr_sub_level_hme_twr_monthly;
!echo "Commencing temp table ext_cdrdm.cdr_sub_level_hme_twr_monthly creation";
create table if not exists ext_cdrdm.cdr_sub_level_hme_twr_monthly stored as orc as
select * from (
...
INNER JOIN ext_cdrdm.cdr_dim_cell_site_info_latest_monthly as bb
...
;
!echo "Temp table ext_cdrdm.cdr_sub_level_hme_twr_monthly creation successful";

SET hive.execution.engine=mr;
drop table if exists ext_cdrdm.cdr_sub_level_hme_twr_yearly;
create table if not exists ext_cdrdm.cdr_sub_level_hme_twr_yearly stored as orc as
select * from (
select *, ROW_NUMBER() OVER(PARTITION BY subscriber_no ORDER BY hme_tower DESC) as home_tower from (
select
subscriber_no, 
...
ROW_NUMBER() OVER(PARTITION BY record_opening_date, subscriber_no ORDER BY record_opening_time DESC) as last_tower_b4_sleep
from cdrdm.fact_gprs_cdr          
where record_opening_date >= date_format(cast('${hiveconf:month_end_date}' as date) + interval '-1' year,'YYYY-MM-dd') and record_opening_date <= '${hiveconf:month_end_date}'
and gprs_choice_mask_archive = 21 and eutran_cellid is not null and trim(eutran_cellid) <> '00000000') as aa 
where aa.last_tower_b4_sleep = 1) as aa 
INNER JOIN ext_cdrdm.cdr_dim_cell_site_info_latest_monthly as bb
ON concat(substr(aa.plmn_id,0,3),'-',substr(aa.plmn_id,-3),'-',conv(aa.tracking_area_code,16,10),'-',conv(aa.eutran_cellid,16,10)) = trim(bb.cgi))
...
--************************************************************************************************************
SET hive.execution.engine=tez;	
drop table if exists ext_cdrdm.cdr_base_cstmr_usge_orc_monthly;
!echo "Commencing temp table ext_cdrdm.cdr_base_cstmr_usge_orc_monthly creation";
create table if not exists ext_cdrdm.cdr_base_cstmr_usge_orc_monthly stored as orc as
select 
...
plmn_id,
tracking_area_code
) as aa  LEFT JOIN ext_cdrdm.cdr_dim_cell_site_info_latest_monthly as bb ON concat(substr(aa.plmn_id,0,3),'-',substr(aa.plmn_id,-3),'-',conv(aa.tracking_area_code,16,10),'-',conv(aa.eutran_cellid,16,10)) = trim(bb.cgi)
...
;
SET hive.execution.engine=mr;
insert into ext_cdrdm.cdr_base_cstmr_usge_orc_monthly
...
) as cc LEFT JOIN ext_cdrdm.cdr_dim_cell_site_info_latest_monthly as dd ON trim(cc.first_cell_id) = trim(dd.cgi_hex)
;
insert into ext_cdrdm.cdr_base_cstmr_usge_orc_monthly 
...
and IMSI >= 302370000000000 and IMSI <=302720999999999
) as ee LEFT JOIN ext_cdrdm.cdr_dim_cell_site_info_latest_monthly as ff ON trim(ee.new_node) = trim(ff.cgi_hex)
;

!echo "Temp table ext_cdrdm.cdr_base_cstmr_usge_orc_monthly load successful";
--************************************************************************************************************
--***** Count number of customers per tower and per month end.
--************************************************************************************************************
SET hive.execution.engine=tez;
drop table if exists ext_cdrdm.cdr_base_cstmr_usge_sub_cnt_rolledup_monthly;
!echo "Commencing temp table ext_cdrdm.cdr_base_cstmr_usge_sub_cnt_rolledup_monthly creation";
create table if not exists ext_cdrdm.cdr_base_cstmr_usge_sub_cnt_rolledup_monthly stored as orc as
select 
date_add(last_day(add_months(record_opening_date, -1)),1) as month_ending,
locationco,
count(distinct subscriber_no) as sub_cnt
from ext_cdrdm.cdr_base_cstmr_usge_orc_monthly
group by date_add(last_day(add_months(record_opening_date, -1)),1),locationco;

!echo "Temp table ext_cdrdm.cdr_base_cstmr_usge_sub_cnt_rolledup_monthly creation successful";

--************************************************************************************************************
--*****Join Customer Data with Home Tower Data and pick up ecid from hash_lookup.ecid_uuid_ban_can
--************************************************************************************************************

drop table if exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_monthly;
!echo "Commencing temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_monthly creation";
create table if not exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_monthly stored as orc as
...
(select * from ext_cdrdm.cdr_base_cstmr_usge_orc_monthly where subscriber_no is not NULL) as customer
LEFT JOIN (select * from ext_cdrdm.cdr_sub_level_hme_twr_monthly where subscriber_no is not NULL and locationco is not NULL) as hme_twr 
...
!echo "Temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_monthly creation successful";
--************************************************************************************************************
--*****Pull NPS raw data from nps_raw.NPS_CONSOLIDATED
--************************************************************************************************************

drop table ext_cdrdm.cdr_cstmr_ltr_raw_monthly;
create table ext_cdrdm.cdr_cstmr_ltr_raw_monthly as 
select
distinct
customer_id,
nps_score,
...
end >= concat("'",date_format(cast('${hiveconf:month_start_date}' as date) + interval '-1' year,'YYYY-MM-dd'),"'")
--'2018-01-01'
;

drop table ext_cdrdm.cdr_cstmr_ltr_ecid_monthly;
create table ext_cdrdm.cdr_cstmr_ltr_ecid_monthly as 
select
qry.*,
hme_twr.locationco,
ecid_sub_map.ecid
...
from ext_cdrdm.cdr_cstmr_ltr_raw_monthly a
left join
...
concat("'",cast(year('${hiveconf:month_start_date}') as string),'-12-31',"'")
--'2019-12-31' 
...
left join
ext_cdrdm.cdr_sub_level_hme_twr_yearly hme_twr
...
--************************************************************************************************************
--*****Join cdr_cstmr_hme_twr_ecid_monthly with UCAR (on ecid) to get msf and nps
--************************************************************************************************************
SET hive.execution.engine=mr;
drop table if exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_monthly1;
!echo "Commencing temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_monthly1 creation";
create table if not exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_monthly1 stored as orc as
select customer.*,
date_add(last_day(add_months(record_opening_date, -1)),1) as month_ending,
...
ltr.avg_nps
from 
(select * from ext_cdrdm.cdr_cstmr_hme_twr_ecid_monthly) as customer
...
(select max(ucar_latest.rundt) from ucar.ucar_final_ecid_mgd ucar_latest where ucar_latest.rundt >= '${hiveconf:month_start_date}' and ucar_latest.rundt <= '${hiveconf:month_end_date}')) 
as ucar_dim ON customer.ecid = ucar_dim.ecid
LEFT JOIN 
(select ecid, avg(nps_score) as avg_nps from ext_cdrdm.cdr_cstmr_ltr_ecid_monthly
group by ecid) ltr
ON customer.ecid = ltr.ecid;
!echo "Temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_monthly1 creation successful";
--************************************************************************************************************
--*****Join cdr_cstmr_hme_twr_ecid_monthly with MLMODEL (on ecid) to get churn score
--************************************************************************************************************
SET hive.execution.engine=tez;
!echo "Commencing temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_monthly creation";
drop table if exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_monthly;
create table if not exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_monthly stored as orc as
select customer.*,
churn.model_score as model_score
from 
(select * from ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_monthly1) as customer
...
;
!echo "Temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_monthly creation successful";
--************************************************************************************************************
...
--************************************************************************************************************
drop table if exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_monthly;
!echo "Commencing temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_monthly creation";
create table if not exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_monthly stored as orc AS
SELECT
month_ending,
ecid,
sum(duration) as mou,
...
FROM ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_monthly
GROUP BY month_ending, ecid;
!echo "Temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_monthly creation successful";
--************************************************************************************************************
--************************************************************************************************************

drop table if exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_monthly;
!echo "Commencing temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_monthly creation";
create table if not exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_monthly stored as orc AS
SELECT
  a.ecid as ecid,
...
INNER JOIN
  ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_monthly b
  ON a.ecid = b.ecid and a.month_ending = b.month_ending;  

!echo "Temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_monthly creation successful";
--************************************************************************************************************
--Create Rolled Up version of cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_monthly
--************************************************************************************************************
drop table if exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup_monthly;
create table if not exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup_monthly stored as orc as
select 
qry.*,
...
from ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_monthly
group by locationco, month_ending) qry
left join 
(select locationco, count(*) as aggr_nps_surveys from ext_cdrdm.cdr_cstmr_ltr_ecid_monthly group by locationco) ltr
on qry.locationco = ltr.locationco;

!echo "Temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup_monthly creation successful";		
--------------------------------------------------------------------------------------------------------------
--************************************************************************************************************
--Creation of temp table for count/percentage of promoters and detractors 
--************************************************************************************************************
drop table if exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup_monthly1;
!echo "Commencing temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup_monthly1 creation";
create table if not exists ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup_monthly1 stored as orc as
select 
month_ending,
locationco, 
...
!echo "Temp table ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup_monthly1 creation successful";	   
---------------------------------------------------------------------------------------------------------------------------------	   
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- 
--************************************************************************************************************
-- Set of Temp tables to calculate below fields for monthly master:
...
drop table if exists ext_cdrdm.port_out_sub_list_monthly;
!echo "Commencing temp table ext_cdrdm.port_out_sub_list_monthly creation";
create table if not exists ext_cdrdm.port_out_sub_list_monthly stored as orc as
select
...
!echo "Temp table ext_cdrdm.port_out_sub_list_monthly creation successful"; 
--************************************************************************************************************
-- get the home towers of the subscribers who ported out - monthly run
--************************************************************************************************************

drop table if exists ext_cdrdm.sub_hme_twr_port_out_monthly;
!echo "Commencing temp table ext_cdrdm.sub_hme_twr_port_out_monthly creation";
create table if not exists ext_cdrdm.sub_hme_twr_port_out_monthly stored as orc as
select * from (
select a.*, 
b.locationco,
b.site_name,
...
b.hme_tower,
b.home_tower
from ext_cdrdm.port_out_sub_list_monthly as a 
LEFT JOIN  ext_cdrdm.cdr_sub_level_hme_twr_monthly as b 
...
!echo "Temp table ext_cdrdm.sub_hme_twr_port_out_monthly creation successful";
--************************************************************************************************************
-- roll up counts of port outs on month_end + tower_id level - monthly run
--************************************************************************************************************

drop table if exists ext_cdrdm.port_out_monthly_rolledup;
!echo "Commencing temp table ext_cdrdm.port_out_monthly_rolledup creation";
create table if not exists ext_cdrdm.port_out_monthly_rolledup stored as orc as
select month_ending, locationco,sum(churn_ind) as cnt_total
from ext_cdrdm.sub_hme_twr_port_out_monthly
group by month_ending, locationco;
...
!echo "Temp table ext_cdrdm.port_out_monthly_rolledup creation successful";
--************************************************************************************************************
...
SELECT 
  ...
  LEFT JOIN ext_cdrdm.port_out_monthly_rolledup po
  on mref.month_end_date = po.month_ending and mref.tower_id = po.locationco 
  LEFT JOIN ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup_monthly ct
  on mref.month_end_date = ct.month_ending and mref.tower_id = ct.locationco
  LEFT JOIN ext_cdrdm.cdr_cstmr_hme_twr_ecid_msf_spread_pertwr_rolledup_monthly1 ltr
  on mref.month_end_date = ltr.month_ending and mref.tower_id = ltr.locationco
  LEFT JOIN ext_cdrdm.cdr_base_cstmr_usge_sub_cnt_rolledup_monthly scn
...
drop table if exists ext_cdrdm.sub_hme_twr_port_out_monthly;
drop table if exists ext_cdrdm.port_out_monthly_rolledup;
drop table if exists ext_cdrdm.cdr_cstmr_ltr_raw_monthly;
drop table if exists ext_cdrdm.cdr_cstmr_ltr_ecid_monthly;
drop table if exists ext_cdrdm.cdr_sub_level_hme_twr_yearly;
