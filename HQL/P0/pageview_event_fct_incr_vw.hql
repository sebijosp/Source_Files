CREATE OR REPLACE VIEW `iptv`.`pageview_event_fct_incr_vw` AS WITH REF_TBL AS (
SELECT
...
AND   `ref_tbl`.`iptv_source_table`   = 'IPTV.PAGEVIEW_EVENT_FCT'
),
BDM_CONTROL AS (
SELECT
...
FROM RAW_FILTERED R
INNER JOIN `IPTV`.`PAGEVIEW_EVENT_FCT` `F`
