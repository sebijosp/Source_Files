CREATE OR REPLACE VIEW `iptv`.`search_message_fct_incr_vw` AS WITH REF_TBL AS (
SELECT
...
AND   `ref_tbl`.`iptv_source_table`   = 'IPTV.SEARCH_MESSAGE_FCT'
),
BDM_CONTROL AS (
SELECT
...
FROM RAW_FILTERED R
INNER JOIN `IPTV`.`SEARCH_MESSAGE_FCT` `F`
