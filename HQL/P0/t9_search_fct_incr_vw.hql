CREATE OR REPLACE VIEW `iptv`.`t9_search_fct_incr_vw` AS WITH REF_TBL AS (
SELECT
...
AND   `ref_tbl`.`iptv_source_table`   = 'IPTV.T9_SEARCH_FCT'
),
BDM_CONTROL AS (
SELECT
...
FROM RAW_FILTERED R
INNER JOIN `IPTV`.`T9_SEARCH_FCT` `F`
