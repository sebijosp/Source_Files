CREATE OR REPLACE VIEW `iptv`.`t9_search_raw_incr_vw` AS WITH REF_TBL AS (
SELECT
...
AND   `ref_tbl`.`iptv_source_table`   = 'IPTV.T9_SEARCH_RAW'
),
BDM_CONTROL AS (
SELECT
...
`compact`.`position`,
`compact`.`programid`,
`compact`.`searchentitytype`,
`compact`.`trackingmetrictype`,
